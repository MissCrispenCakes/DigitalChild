...
def init_driver(headless=True):
    ...
    chrome_binary = shutil.which("google-chrome") or shutil.which("chromium-browser") or shutil.which("chromium")
    if not chrome_binary:
        raise RuntimeError("No Chrome/Chromium binary found. Install with `sudo apt install chromium-browser`")

    # Try to detect chromedriver
    chromedriver_path = shutil.which("chromedriver")
    if not chromedriver_path:
        raise RuntimeError("No chromedriver found. Install with `sudo apt install chromium-chromedriver` "
