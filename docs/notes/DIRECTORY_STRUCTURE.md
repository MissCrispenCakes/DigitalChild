...
Flask REST API (Phase 4) exposing the pipeline data over HTTP — 14 endpoints.
**HAS `__init__.py`** (proper package). Run with `python run_api.py`.

- `app.py`, `config.py`, `extensions.py` → app factory, config, shared extensions
- `routes/` → endpoint blueprints: documents, scorecard, tags, timeline, export, health
- `services/` → data-access layer: metadata, scorecard, tags, timeline, export
- `middleware/` → auth (API keys), rate limiting, error handlers
- `utils/` → response shaping, cache keys, request validators

______________________________________________________________________

## Utilities (`utils/`)

Standalone scripts and helpers (run directly; not part of the pipeline package).

- `build_scorecard_viz_data.py` → **read-only** generator → `docs/scorecard/data/scorecard.json` (drives the live scorecard map/charts/explorer)
- `build_transparency_watch_data.py` → **read-only** generator → `docs/transparency-watch/data/transparency.json` (drives the Source Transparency Watch; Wayback CDX engine)
- `export_scorecard_formats.py` → regenerates root convenience copies (`scorecard.{xlsx,ods,csv,gsheet.json}`) from `scorecard_main.xlsx`
- `iso_mapping.py`, `detectors.py`, `clean_names.py`, `get_names.py` → naming / ISO / detection helpers
- `combinePDFs.py`, `pipeline_runner_DEMO.py` → PDF + demo utilities
- `add_spdx_headers.py`, `setup_reuse.py`, `update_csv_licenses.py` → licensing / REUSE tooling

> The two `build_*` viz/watch generators write **only** into `docs/` — they never modify canonical sources.
...
Comprehensive documentation (60+ markdown files) plus the public GRIMdata.org site.
...
### Website & Visualizations (`docs/website/`, `docs/scorecard/`, `docs/transparency-watch/`)

The public GRIMdata.org site (Material for MkDocs), deployed to GitHub Pages on push to `basecamp`.

- `website/` → home, getting-started, project pages; `javascripts/` (extra.js, scorecard.js, transparency.js) and `stylesheets/extra.css`
- `scorecard/` → overview, design, data-access, visualization, explorer; `data/scorecard.json` is the published viz dataset (generated by `utils/build_scorecard_viz_data.py`)
- `transparency-watch/` → Source Transparency Watch page; `data/transparency.json` is the published dataset (generated by `utils/build_transparency_watch_data.py`)

### Maintenance (`docs/maintenance/`)

- `SOURCE_AVAILABILITY_LOG.md` → record of previously-available external sources now gone (link rot) — the inverse of the Transparency Watch
- `SCORECARD_*` → scorecard update guides, maintenance reports, and change logs
