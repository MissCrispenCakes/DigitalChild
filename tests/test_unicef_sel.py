...
"""
Tests for UNICEF Selenium Scraper

Verifies that unicef_sel scraper:
...
import scrapers.unicef_sel as unicef_sel


class TestUNICEFSeleniumScraper:
    """Test suite for unicef_sel scraper."""

    def test_scrape_accepts_base_url(self):
        """Test that scrape() accepts base_url parameter."""
        with tempfile.TemporaryDirectory() as tmpdir:
            original_raw_dir = unicef_sel.RAW_DIR
            unicef_sel.RAW_DIR = tmpdir
            ...
            try:
                with mock.patch("scrapers.unicef_sel.init_driver") as mock_init:
                    with mock.patch("scrapers.unicef_sel.WebDriverWait"):
                        mock_init.return_value = mock_driver
                        result = unicef_sel.scrape(base_url="https://example.com")
                        assert isinstance(result, list)
                        mock_driver.quit.assert_called_once()
            finally:
                unicef_sel.RAW_DIR = original_raw_dir
    ...
    def test_scrape_handles_driver_error(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            try:
                with mock.patch(
                    "scrapers.unicef_sel.init_driver"
                ) as mock_init:
            ...
            try:
                with mock.patch("scrapers.unicef_sel.init_driver") as mock_init:
                    with mock.patch("scrapers.unicef_sel.WebDriverWait"):
                        mock_init.return_value = mock_driver
                        with mock.patch(
                            "scrapers.unicef_sel.download_file"
                        ) as mock_download:
...
class TestUNICEFSeleniumReturnValue:
    """Test return value consistency."""

    def test_return_type_consistency(self):
        """Verify that unicef_sel returns list consistently."""
        with tempfile.TemporaryDirectory() as tmpdir:
            original_raw_dir = unicef_sel.RAW_DIR
            unicef_sel.RAW_DIR = tmpdir
