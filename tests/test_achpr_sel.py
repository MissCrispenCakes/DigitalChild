...
"""
Tests for ACHPR Selenium Scraper

Verifies that achpr_sel scraper:
...
import scrapers.achpr_sel as achpr_sel


class TestACHPRSeleniumScraper:
    """Test suite for achpr_sel scraper."""

    def test_scrape_accepts_base_url(self):
        """Test that scrape() accepts base_url parameter."""
        with tempfile.TemporaryDirectory() as tmpdir:
            original_raw_dir = achpr_sel.RAW_DIR
            achpr_sel.RAW_DIR = tmpdir
            ...
            try:
                with mock.patch("scrapers.achpr_sel.init_driver") as mock_init:
                    with mock.patch("scrapers.achpr_sel.WebDriverWait"):
                        mock_init.return_value = mock_driver
                        result = achpr_sel.scrape(base_url="https://example.com")
                        assert isinstance(result, list)
                        mock_driver.quit.assert_called_once()
            finally:
                achpr_sel.RAW_DIR = original_raw_dir
    ...
    def test_scrape_handles_driver_error(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            try:
                with mock.patch(
                    "scrapers.achpr_sel.init_driver"
                ) as mock_init:
            ...
            try:
                with mock.patch("scrapers.achpr_sel.init_driver") as mock_init:
                    with mock.patch("scrapers.achpr_sel.WebDriverWait"):
                        mock_init.return_value = mock_driver
                        with mock.patch(
                            "scrapers.achpr_sel.download_file"
                        ) as mock_download:
...
class TestACHPRSeleniumReturnValue:
    """Test return value consistency."""

    def test_return_type_consistency(self):
        """Verify that achpr_sel returns list consistently."""
        with tempfile.TemporaryDirectory() as tmpdir:
            original_raw_dir = achpr_sel.RAW_DIR
            achpr_sel.RAW_DIR = tmpdir
