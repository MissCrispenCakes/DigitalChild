...
"""
Tests for UNICEF Scraper

Verifies that unicef scraper:
...
class TestUNICEFScraper:
    """Test suite for unicef scraper."""

    def test_scrape_accepts_base_url(self):
        """Test that scrape() accepts base_url parameter."""
        with mock.patch("scrapers.unicef.requests.get") as mock_get:
            mock_get.return_value.status_code = 200
            mock_get.return_value.text = "<html><body></body></html>"
            with tempfile.TemporaryDirectory() as tmpdir:
                original_raw_dir = unicef.RAW_DIR
                unicef.RAW_DIR = tmpdir
                try:
                    result = unicef.scrape(base_url="https://example.com/test.pdf")
                    assert isinstance(result, list)
                finally:
                    unicef.RAW_DIR = original_raw_dir
...
class TestUNICEFReturnValue:
    """Test return value consistency."""

    def test_return_type_consistency(self):
        """Verify that unicef returns list consistently."""
        with mock.patch("scrapers.unicef.requests.get") as mock_get:
            mock_get.return_value.status_code = 200
            mock_get.return_value.text = "<html><body></body></html>"
            with tempfile.TemporaryDirectory() as tmpdir:
